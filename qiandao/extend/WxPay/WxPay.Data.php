...
class WxPayDataBase
{
	...
	**/
    ...
    /**
     * 设置签名，详见签名生成算法(兼容团购退款)
     * @param string $value
     **/
    public function gSetSign($tenantId)
    {
        $sign = $this->gMakeSign('',$tenantId);
        $this->values['sign'] = $sign;
        return $sign;
    }
	...
	**/
	 ...
	 */
	public function MakeSign($mch_id='')
	{
        if(!empty($mch_id)){
            $tenant = \app\admin\model\Tenant::where('mchId','=',$mch_id)->find();
            $key = $tenant->mchKey;
        }else{
            $key = WxPayConfig::getTenantConfig()['KEY'];
        }
		//签名步骤一：按字典序排序参数
		ksort($this->values);
     ...
     * @return 签名，本函数不覆盖sign成员变量，如要设置签名需要调用SetSign方法赋值
     */
    public function gMakeSign($mch_id='',$tenantId)
    {
        $key = WxPayConfig::getTenantConfig($tenantId)['KEY'];
...
class WxPayResults extends WxPayDataBase
{
	 ...
	 * 检测签名
	 */
	public function CheckSign($mch_id='')
	{
		//fix异常
		if(!$this->IsSignSet()){
     ...
     * 检测签名(兼容团购退款)
     */
    public function gCheckSign($mch_id='',$tenantId)
    {
        //fix异常
        if(!$this->IsSignSet()){
            throw new WxPayException("签名错误！");
        ...
        $sign = $this->gMakeSign($mch_id,$tenantId);
        if($this->GetSign() == $sign){
            return true;
        }
        throw new WxPayException("签名错误！");
	...
	{	
		...
		$obj->CheckSign($obj->GetValues()['mch_id']);
        return $obj->GetValues();
	}
     ...
     * @throws WxPayException
     */
    public static function gInit($xml,$tenantId)
    {
        $obj = new self();
        $obj->FromXml($xml);
        //fix bug 2015-06-29
        ...
        $obj->gCheckSign($obj->GetValues()['mch_id'],$tenantId);
        return $obj->GetValues();
    }

    /**
     * 将xml转为array(兼容企业向用户打款)
     * @param string $xml
     * @throws WxPayException
     */
    public static function Init_t($xml)
    {
        $obj = new self();
        $obj->FromXml($xml);
        //fix bug 2015-06-29
        if($obj->values['return_code'] != 'SUCCESS' || $obj->values['result_code'] != 'SUCCESS'){
            return $obj->GetValues();
        }
        //提现成功后不验证签名
        //$obj->CheckSign($obj->GetValues()['mchid']);
        return $obj->GetValues();
    }
}
